...
const avgPrice = computed(() => {
  ...
  const avg =
    data.value.reduce((s, r) => s + (r.price || 0), 0) / data.value.length;
...
async function save() {
  ...
  try {
    ...
    if (modal.isEdit) await updateRecord("/MenuItem/Update", form.id, payload);
    else await createRecord("/MenuItem/Create", payload);
    modal.open = false;
